...
import { SrverHeader } from "./server-header";
...
import { SrverMembers } from "./server-member";
...
export const ServerSidebar: React.FC<ServerSidebarProps> = async ({
  ...
  return (
    <div className="flex flex-col h-full text-primary w-full dark:bg-[#2B2D31] bg-[#F2F3F5]">
      <SrverHeader server={server} role={role} />
        ...
        {!!members?.length && (
          <div className="mb-2">
            ...
            <div className="space-y-[2px] mt-2">
              {members.map((member) => (
                <SrverMembers key={member.id} member={member} server={server} />
              ))}
            </div>
          </div>
        )}
